-- Based on https://raw.githubusercontent.com/torch/nn/master/Euclidean.lua

local Euclidean, parent = torch.class('nn.LazyEuclidean', 'nn.Module')
...
-- ELOD: Apply a factor to the learning rate to make the layer less responsive
function Euclidean:updateParameters(learningRate)
   local params, gradParams = self:parameters()
   if params then
      for i=1,#params do
         params[i]:add(-learningRate*self.learningRateFactor, gradParams[i])
      end
   end
end

-- ELOD: Distribute the points better initially
